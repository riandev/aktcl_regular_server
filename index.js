...
const app = express();
app.use(express.static("../aktcl_regular_client/build"));
app.use(cors());
...
client.connect((err) => {
  const userCollection = client.db("aktcl_regular").collection("users");
  const adminCollection = client.db("aktcl_regular").collection("admins");
  const leadsCollection = client.db("aktcl_regular").collection("leads");
